...
public class Shark {
	...
	public Shark(float _x, float _y, float _z, float _scale, float _tail_speed, Vivarium _v) {
		...
		trans_dir_x = rdm.nextFloat();
		trans_dir_y = rdm.nextFloat();
		trans_dir_z = rdm.nextFloat();
	...
	public void draw(GL2 gl) {
		...
		gl.glColor3f(0.4f, 0.5f, 0.6f); // Blue 
	...
	// Function that draws a bounding sphere for debugging
	...
	// Uses potential function to avoid wall and search fish
	...
	private void translate() {
		...
		// dividing the boundaries by the scale factor for the shark
		// to avoid wall
		if (x > 1.8/scale || x < -1.8/scale ) {
			x = 1.8f/scale;
			if (trans_dir_x > 0) {
				trans_dir_x = -1 * n;
			} else {
		...
		// To search the location of fish
		public void addPrey(Fish f) {
			prey = f;
		}
		
		// If fish is eaten, just walk around
